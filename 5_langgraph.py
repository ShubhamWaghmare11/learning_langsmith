...
@traceable(name="evaluate_language_fn")
...
@traceable(name="evaluate_analysis_fn")
def evaluate_analysis(state: UPSCState):
    prompt = (
        "Evaluate the depth of analysis following essay and provide feedback."
        "and assign a score out of 10 \n\n" + state['essay']
    )
    out = structured_model.invoke(prompt)
    return {"analysis_feedback": out.feedback,"individual_scores":[out.score]}

@traceable(name="evaluate_thought_fn")
...
if __name__ == "__main__":
    result = workflow.invoke(
        {"essay":essay2},
        config={
            "run_name":"Evaluation_upsc_essay",
